<script lang="ts">
	import RangeCalendar from '$lib/calendar/rangepicker/RangeCalendar.svelte';
	import dayjs from 'dayjs';

	let selectedStartDate: Date = dayjs().toDate();
	let selectedEndDate: Date = dayjs().add(2, 'day').toDate();
...
<div class="flex flex-col w-full p-4">
	<div class="flex flex-row justify-between">
		...
		<h2 class="card-sub-heading">
			Selected Date: {dayjs(selectedStartDate).format('YYYY-MM-DD')} - {dayjs(
				selectedEndDate
			).format('YYYY-MM-DD')}
		</h2>
	</div>
				...
				<div class="display">
					<div class="artboard artboard-demo phone-1 p-0 m-0 pt-8">
						<RangeCalendar
							...
							on:select={(e) => {
								selectedStartDate = e.detail.selectedStartDate;
								selectedEndDate = e.detail.selectedEndDate;
							}}
						/>
					</div>
				</div>
